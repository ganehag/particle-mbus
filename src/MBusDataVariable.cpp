...
unsigned long
MBusDataVariableHeader::decodeID() {
    unsigned long val = 0;
    size_t i;
...
void
MBusDataVariable::append_record(MBusDataRecord *record) {
    MBusDataRecord *iter;
...
int
MBusDataVariable::parse(MBusFrame *frame) {
    ...
    if (frame != NULL) {
        ...
        while (i < frame->data_size) {
            ...
            if ((record->drh->vib->vif & MBUS_DIB_VIF_WITHOUT_EXTENSION) == 0x7C) {
                ...
                // MBusUtils::str_decode(record->drh->vib->custom_vif, &(frame->data[i]), var_vif_len);
