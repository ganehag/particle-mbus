...
int
MBusDataRecord::decodeInt(size_t int_data_size, int *value) {
    size_t i;
    int neg;
    *value = 0;
...
int
MBusDataRecord::decodeLong(size_t int_data_size, long *value) {
    size_t i;
    int neg;
    *value = 0;
...
int
MBusDataRecord::decodeLongLong(size_t int_data_size, long long *value) {
    size_t i;
    int neg;
    *value = 0;
...
long long
MBusDataRecord::decodeBCD(size_t bcd_data_size) {
    long long val = 0;
    size_t i;
...
void
MBusDataRecord::decodeStr(unsigned char *dst, const unsigned char *src, size_t len) {
    size_t i;
...
void
MBusDataRecord::decodeBin(unsigned char *dst, const unsigned char *src, size_t len, size_t max_len) {
    size_t i, pos;
...
void
MBusDataRecord::decodeTM(struct tm *t, size_t t_data_size) {
    if (t == NULL) {
        return;
    }
    ...
    if (this->data) {
        ...
        else if (t_data_size == 2)           // Type G: Compound CP16: Date
        {
            ...
            t->tm_year = 100 + (((this->data[0] & 0xE0) >> 5) |
                         ((this->data[1] & 0xF0) >> 1));
        }
    }
};
...
MBusDataRecord::getRecord() {
    ...
    if ((this->drh->dib->dif == MBUS_DIB_DIF_MANUFACTURER_SPECIFIC) ||
        (this->drh->dib->dif == MBUS_DIB_DIF_MORE_RECORDS_FOLLOW)) /* MBUS_DIB_DIF_VENDOR_SPECIFIC */
    {
        ...
        if (this->decodeValue(&value_out_real, &value_out_str, &value_out_str_size) != 0) {
        ...
        if (this->drh->vib->normalize(value_out_real, &(record->unit), &real_val, &(record->quantity)) != 0) {
...
long
MBusDataRecord::getStorageNumber() {
    int bit_index = 0;
    long result = 0;
    ...
    for (unsigned int i=0; i<this->drh->dib->ndife; i++) {
        result |= (this->drh->dib->dife[i] & MBUS_DATA_RECORD_DIFE_MASK_STORAGE_NO) << bit_index;
        bit_index += 4;
    }
...
long
MBusDataRecord::getTariff() {
    int bit_index = 0;
    long result = 0;

    if (this->drh->dib->ndife > 0) {
        for (unsigned int i=0; i < this->drh->dib->ndife; i++) {
            result |= ((this->drh->dib->dife[i] & MBUS_DATA_RECORD_DIFE_MASK_TARIFF) >> 4) << bit_index;
            bit_index += 2;
        }
...
int
MBusDataRecord::getDevice() {
    int bit_index = 0;
    int result = 0;

    if (this->drh->dib->ndife > 0) {
        for (unsigned int i=0; i<this->drh->dib->ndife; i++) {
            result |= ((this->drh->dib->dife[i] & MBUS_DATA_RECORD_DIFE_MASK_DEVICE) >> 6) << bit_index;
            bit_index++;
        }
...
int
MBusDataRecord::decodeValue(double *value_out_real, char **value_out_str, int *value_out_str_size) {
    int result = 0;
    ...
    *value_out_str_size = 0;
    ...
    switch (this->drh->dib->dif & MBUS_DATA_RECORD_DIF_MASK_DATA) {
        ...
        case 0x04: /* 4 byte integer (32 bit) */
            ...
            if ( (vif == 0x6D) ||
                ((this->drh->vib->vif == 0xFD) && (vife == 0x30)) ||
        ...
        case 0x0D: /* variable length */
            if (this->data_len <= 0xBF) {
                ...
                *value_out_str_size = this->data_len;
                this->decodeStr((unsigned char*)(*value_out_str), this->data, this->data_len);
                result = 0;
                break;
            }
        ...
        case 0x0F: /* Special functions */
            ...
            *value_out_str_size = 3 * this->data_len;
            this->decodeBin((unsigned char*)(*value_out_str), this->data, this->data_len, (3 * this->data_len + 1));
            result = 0;
            break;
