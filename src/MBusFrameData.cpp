...
int MBusFrameData::parse(MBusFrame *frame) {
  char direction;
  ...
  if (direction == MBUS_CONTROL_MASK_DIR_S2M) {
    if (frame->control_information == MBUS_CONTROL_INFO_ERROR_GENERAL) {
      ...
      // snprintf(error_str, sizeof(error_str), "Unknown control information
      // 0x%.2x", frame->control_information);

      return -1;
    }
  }

  // snprintf(error_str, sizeof(error_str), "Wrong direction in frame (master to
  // slave)");
