...
MBus::MBus() {
    MBUS_DEBUG("MBus Create\n");

};

MBus::~MBus() {
    MBUS_DEBUG("MBus Destroy\n");    
};

int
MBus::isPrimaryAddress(int value) {
    return ((value >= 0x00) && (value <= 0xFF));
};

int
MBus::isSecondaryAddress(const char * value) {
    int i;
...
int
MBus::initSlaves(MBusHandle *handle) {
...
int
MBus::selectSecondaryAddress(MBusHandle *handle, const char *mask) {
    int ret;
    MBusFrame reply;
...
int
MBusSerialHandle::open() {
    return this->setBaudrate(2400);
};

int
MBusSerialHandle::close() {
    if(this->handle != NULL) {
        this->handle->end();
        return 0;
    }
...
int
MBusSerialHandle::setBaudrate(long baudrate) {
    if(this->handle != NULL) {
        this->handle->flush();
        this->handle->end();
...
int
MBusSerialHandle::send(MBusFrame *frame) {
...
int
MBusSerialHandle::sendPingFrame(int address, bool purge_response) {
    int retval = 0;
    MBusFrame *frame;

    if (MBusUtils::is_primary_address(address) == 0) {
        return 1;
    }
...
int
MBusSerialHandle::requestSendRecv(int address, MBusFrame *reply, int max_frames) {
    ...
    while (more_frames) {
        ...
        // Is this needed? Or will reply_data clean it up anyway? When it does recursive child delete at end of scope?
