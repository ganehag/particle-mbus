...
int
MBusFrame::calc_length() {
    this->length1 = this->length2 = this->get_length();
    return 0;
};

size_t
MBusFrame::get_length() {
    switch(this->type) {
        case MBUS_FRAME_TYPE_CONTROL:
            return 3;
        case MBUS_FRAME_TYPE_LONG:
...
int
MBusFrame::parse(unsigned char *data, size_t data_size) {
    size_t i, len;
...
MBusDataVariable *MBusFrame::getVariableData() {
    ...
    // FIXME
    if (this->control_information != MBUS_CONTROL_INFO_RESP_VARIABLE) {
        // snprintf(error_str, sizeof(error_str), "Non-variable data response (has no records?).");
        return NULL;
    }
    ...
    return data_var; // Return the root (delete, needs to be managed on the other side)
}
    ...
    switch (this->type) {
        ...
        case MBUS_FRAME_TYPE_SHORT:
            ...
            if ((this->control !=  MBUS_CONTROL_MASK_SND_NKE)                          &&
                ...
                (this->control != (MBUS_CONTROL_MASK_REQ_UD1 | MBUS_CONTROL_MASK_FCB)) &&
                (this->control !=  MBUS_CONTROL_MASK_REQ_UD2)                          &&
                (this->control != (MBUS_CONTROL_MASK_REQ_UD2 | MBUS_CONTROL_MASK_FCB))) {
                // snprintf(error_str, sizeof(error_str), "Unknown Control Code 0x%.2x", this->control);
        ...
        case MBUS_FRAME_TYPE_LONG:
            ...
            if ((this->control !=  MBUS_CONTROL_MASK_SND_UD)                          &&
                ...
                (this->control != (MBUS_CONTROL_MASK_RSP_UD | MBUS_CONTROL_MASK_DFC | MBUS_CONTROL_MASK_ACD)))
            {
                // snprintf(error_str, sizeof(error_str), "Unknown Control Code 0x%.2x", this->control);
...
int
MBusFrame::pack(unsigned char *data, size_t data_size) {
   size_t i, offset = 0;
...
int
MBusFrame::selectSecondaryPack(const char *address) {
    int val, i, j, k;
    char tmp[16];
    ...
    if (MBusUtils::is_secondary_address(address) == 0) {
        // snprintf(error_str, sizeof(error_str), "%s: address is invalid.", __PRETTY_FUNCTION__);
        return -1;
    }

    this->control = MBUS_CONTROL_MASK_SND_UD | MBUS_CONTROL_MASK_DIR_M2S | MBUS_CONTROL_MASK_FCB;
