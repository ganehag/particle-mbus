...
int
MBusValueInformationBlock::vif_normalize(int vif, double value, char **unit_out, double *value_out, char **quantity_out) {
    int i;
    double exponent = 1.0;
...
int
MBusValueInformationBlock::normalize(double value, char **unit_out, double *value_out, char **quantity_out) {
    int code;
    ...
    if (this->vif == 0xFD) /* first type of VIF extention: see table 8.4.4 a */
    {
        ...
        if (this->vif_normalize(code, value, unit_out, value_out, quantity_out) != 0)
        {
    ...
    if ((this->vif & MBUS_DIB_VIF_EXTENSION_BIT) &&
        (this->vif != 0xFD) &&
